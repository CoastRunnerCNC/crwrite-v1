  ...
  const handleClick = () => {
    ...
    if (props.home) {
      ...
      if (position) {
        ...
        ipcRenderer.send("CNC::ExecuteCommand", `G0 G53 X${position.x[props.units]} Y${position.y[props.units]}`);
        ipcRenderer.send("CNC::ExecuteCommand", `G0 G53 Z${position.z[props.units]}`);
      } else {
        // throw set position popup
        setShowPopup(true);
      }
